...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games w ON w.winner_id = team_id LEFT JOIN games o ON o.opponent_id = team_id WHERE w.round = 'Eighth-Final' AND w.year = 2014 OR o.round = 'Eighth-Final' AND o.year = 2014 ORDER BY name")"
